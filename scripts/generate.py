...
_PROTECTED = {"scripts/generate.py"}
    ...
    for d in DIR_EXTENSIONS:
        ...
        if p.exists():
            for f in p.iterdir():
                ...
                if f.is_file() and not f.name.startswith(".") and rel not in _PROTECTED:
                    existing.append(f)
...
def _write(path: Path) -> None:
    ...
    content = _GENERATORS.get(ext, _txt)()
...
def main() -> None:
    n = max(1, min(int(os.environ.get("COMMIT_COUNT", "1")), 5))
    for i in range(n):
        path = _pick_path()
        _write(path)
        ...
        msg = random.choice(
            COMMIT_TEMPLATES[
                random.choices(list(COMMIT_TEMPLATES.keys()), weights=_WEIGHTS, k=1)[0]
            ]
        )
        _git("commit", "-m", msg)
        print(f"[{i+1}/{n}] {msg} -> {rel}")
